document.addEventListener('DOMContentLoaded', () => {
    ...
    let renderAllShelves = () => {}; // No-op function by default, will be replaced by page-specific logic
    ...
    const parseMDHighlights = (mdContent) => {
        const highlights = []; let title = 'Unknown Title';
        const frontmatterMatch = mdContent.match(/---\s*title:\s*"(.*?)"\s*---/);
        if (frontmatterMatch && frontmatterMatch[1]) title = frontmatterMatch[1];
    ...
    const parseHTMLHighlights = (htmlContent) => {
        const doc = new DOMParser().parseFromString(htmlContent, 'text/html');
        const title = doc.querySelector('.bookTitle')?.textContent.trim() || 'Unknown Title';
        const highlights = Array.from(doc.querySelectorAll('.noteText')).map(el => el.textContent.trim());
        return { title, highlights };
    };
    ...
    if (document.getElementById('details-page-content')) {
        ...
        const renderDetailsPage = (book) => { /* ... unchanged ... */ if (!book) { detailsContainer.innerHTML = `<p class="text-center">Book not found.</p> <a href="/" class="block text-center mt-4 text-indigo-600 hover:underline">&larr; Back to Library</a>`; return; } const coverUrl = book.imageLinks?.thumbnail || `https://placehold.co/128x192/e2e8f0/475569?text=N/A`; const authors = book.authors ? book.authors.join(', ') : 'Unknown Author'; const pageCount = book.pageCount ? `${book.pageCount} pages` : 'N/A'; const publishedDate = book.publishedDate ? new Date(book.publishedDate).getFullYear() : 'N/A'; const isbn = book.industryIdentifiers?.find(id => id.type === 'ISBN_13')?.identifier || book.industryIdentifiers?.find(id => id.type === 'ISBN_10')?.identifier || 'N/A'; const goodreadsLink = `https://www.goodreads.com/search?q=${encodeURIComponent(book.title + ' ' + (book.authors ? book.authors[0] : ''))}`; detailsContainer.innerHTML = `<div class="mb-6"><a href="/" class="text-indigo-600 hover:underline">&larr; Back to Library</a></div><div class="bg-white p-6 rounded-xl shadow-md"><h2 class="text-3xl font-bold mb-2">${book.title}</h2><p class="text-slate-600 mb-6 text-xl">by ${authors}</p><div class="flex flex-col md:flex-row gap-8"><div class="w-40 flex-shrink-0 text-center"><img src="${coverUrl}" alt="Cover" class="w-40 h-60 object-contain rounded-md shadow-md mx-auto"></div><div class="flex-1"><div class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-slate-700 mb-6 p-4 bg-slate-50 rounded-md border"><div><strong>Published:</strong> ${publishedDate}</div><div><strong>Pages:</strong> ${pageCount}</div><div><strong>ISBN:</strong> ${isbn}</div><div><a href="${goodreadsLink}" target="_blank" class="text-indigo-600 hover:underline font-semibold">Find on Goodreads</a></div></div><div id="reading-log-container" data-book-id="${book.id}" data-shelf="${book.shelf}"></div><div class="mt-6"><h4 class="text-lg font-semibold">Highlights</h4><div id="highlights-container" class="mt-2"></div></div></div></div></div>`; renderReadingLogView(book.id, book.shelf); renderHighlights(book.id, book.shelf);};
        const renderReadingLogView = (bookId, shelf) => { /* ... unchanged ... */ const container = document.getElementById('reading-log-container'); const book = library[shelf]?.find(b => b.id === bookId); if (!container || !book) return; let durationHTML = ''; if (shelf === 'read' && book.startedOn && book.finishedOn) { const start = new Date(book.startedOn); const end = new Date(book.finishedOn); const diffTime = Math.abs(end - start); const diffDays = Math.max(1, Math.ceil(diffTime / (1000 * 60 * 60 * 24))); durationHTML = `<div class="flex justify-between items-center"><span class="font-semibold">Time to Finish:</span><span>${diffDays} day${diffDays !== 1 ? 's' : ''}</span></div>`; } container.innerHTML = `<div class="flex justify-between items-center"><h4 class="text-lg font-semibold">Reading Log</h4><button id="edit-log-btn" class="text-sm text-indigo-600 hover:underline font-semibold">Edit</button></div><div class="space-y-2 mt-2 text-sm text-slate-700 p-3 bg-slate-50 rounded-md border"><div class="flex justify-between items-center"><span class="font-semibold">Medium:</span><span>${book.readingMedium || 'Not set'}</span></div><div class="flex justify-between items-center"><span class="font-semibold">Started On:</span><span>${book.startedOn || 'Not set'}</span></div>${shelf === 'read' ? `<div class="flex justify-between items-center"><span class="font-semibold">Finished On:</span><span>${book.finishedOn || 'Not set'}</span></div>${durationHTML}` : ''}</div>`;};
        const renderReadingLogEdit = (bookId, shelf) => { const container = document.getElementById('reading-log-container'); const book = library[shelf]?.find(b => b.id === bookId); if (!container || !book) return; const mediums = ["Paperback", "Kindle Paperwhite", "Mobile", "Tablet"]; const mediumOptions = mediums.map(m => `<option value="${m}" ${book.readingMedium === m ? 'selected' : ''}>${m}</option>`).join(''); container.innerHTML = `<h4 class="text-lg font-semibold">Reading Log</h4><div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2 text-sm text-slate-700 p-3 bg-slate-50 rounded-md border"><div><label for="reading-medium" class="font-semibold block mb-1">Medium</label><select id="reading-medium" class="w-full p-2 border rounded-md bg-white"><option value="">Not set</option>${mediumOptions}</select></div><div><label for="started-on" class="font-semibold block mb-1">Started On</label><input type="date" id="started-on" value="${book.startedOn || ''}" class="w-full p-2 border rounded-md bg-white"></div>${shelf === 'read' ? `<div><label for="finished-on" class="font-semibold block mb-1">Finished On</label><input type="date" id="finished-on" value="${book.finishedOn || ''}" class="w-full p-2 border rounded-md bg-white"></div>` : ''}</div><div class="flex justify-end gap-2 mt-4"><button id="cancel-log-edit-btn" class="text-sm bg-slate-200 text-slate-700 font-semibold py-2 px-3 rounded-md hover:bg-slate-300">Cancel</button><button id="save-log-btn" class="text-sm bg-indigo-600 text-white font-semibold py-2 px-3 rounded-md hover:bg-indigo-700">Save Changes</button></div>`;};
        const renderHighlights = (bookId, shelf) => { /* ... unchanged ... */ const container = document.getElementById('highlights-container'); const book = library[shelf]?.find(b => b.id === bookId); if (!container || !book) return; if (book.highlights && book.highlights.length > 0) { container.innerHTML = '<ul class="list-disc list-inside space-y-2 text-slate-600">' + book.highlights.map(h => `<li class="border-l-4 border-amber-300 pl-3">${h}</li>`).join('') + '</ul>'; } else { container.innerHTML = '<p class="text-slate-500">No highlights imported for this book.</p>'; }};
        ...
        detailsContainer.addEventListener('click', async (e) => {
            ...
            if (e.target.id === 'save-log-btn') {
                ...
                if (book) {
                    book.readingMedium = container.querySelector('#reading-medium').value;
                    book.startedOn = container.querySelector('#started-on').value;
                    const finishedOnInput = container.querySelector('#finished-on');
                    ...
                    requestPassword(async () => {
                        ...
                        if (success) renderReadingLogView(bookId, shelf);
                    });
                }
            }
        });
    ...
    if (highlightsFileInput) highlightsFileInput.addEventListener('change', (e) => {
        ...
        reader.onload = (event) => {
            ...
            if (file.name.endsWith('.md')) {
                parsed = parseMDHighlights(event.target.result);
            } else if (file.name.endsWith('.html')) {
                parsed = parseHTMLHighlights(event.target.result);
            } else {
                showToast('Unsupported file type. Please use .html or .md', 'error');
                return;
            }
    ...
    if (confirmImportBtn) confirmImportBtn.addEventListener('click', () => {
        ...
        const [shelf, bookId] = selected.split(':');
        const book = library[shelf]?.find(b => b.id === bookId);
    ...
    if (forceRefreshBtn) forceRefreshBtn.addEventListener('click', () => { localStorage.removeItem(CACHE_KEY); getLibrary(true).then(lib => { if(lib) renderAllShelves(); }); closeModal(document.getElementById('library-modal')); });

    // ... other data management listeners (json import/export) would go here
});
