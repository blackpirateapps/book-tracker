document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.getElementById('main-dashboard')) {
        ...
        const handleSearch = async (e) => {
            ...
            try {
                const response = await fetch(`https://openlibrary.org/search.json?q=${encodeURIComponent(searchTerm)}&limit=18`);
                if (!response.ok) throw new Error('Network error with Open Library');
                ...
                const formattedBooks = data.docs.map(doc => {
                    ...
                    return {
                        ...
                        imageLinks: doc.cover_i ? { thumbnail: `https://covers.openlibrary.org/b/id/${doc.cover_i}-L.jpg` } : {},
                ...
                searchMessage.textContent = 'Failed to fetch books from Open Library.';
            }
        };
    ...
    if (document.getElementById('details-page-content')) {
        ...
        const renderDetailsPage = (book) => { if (!book) { detailsContainer.innerHTML = `<p class="text-center">Book not found.</p> <a href="/dashboard/" class="block text-center mt-4 text-indigo-600 hover:underline">&larr; Back to Dashboard</a>`; return; } const coverUrl = book.imageLinks?.thumbnail || `https://placehold.co/128x192/e2e8f0/475569?text=N/A`; const authors = book.authors ? book.authors.join(', ') : 'Unknown Author'; const pageCount = book.pageCount ? `${book.pageCount} pages` : 'N/A'; const publishedDate = book.publishedDate ? book.publishedDate : 'N/A'; const isbn = book.industryIdentifiers?.find(id => id.type === 'ISBN_13')?.identifier || book.industryIdentifiers?.find(id => id.type === 'ISBN_10')?.identifier || 'N/A'; const goodreadsLink = `https://www.goodreads.com/search?q=${encodeURIComponent(book.title + ' ' + (book.authors ? book.authors[0] : ''))}`; detailsContainer.innerHTML = `<div class="mb-6"><a href="/dashboard/" class="text-indigo-600 hover:underline">&larr; Back to Dashboard</a></div><div class="bg-white p-6 rounded-xl shadow-md"><h2 class="text-3xl font-bold mb-2">${book.title}</h2><p class="text-slate-600 mb-6 text-xl">by ${authors}</p><div class="flex flex-col md:flex-row gap-8"><div class="w-40 flex-shrink-0 text-center"><img src="${coverUrl}" alt="Cover" class="w-40 h-60 object-contain rounded-md shadow-md mx-auto"></div><div class="flex-1"><div class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-slate-700 mb-6 p-4 bg-slate-50 rounded-md border"><div><strong>Published:</strong> ${publishedDate}</div><div><strong>Pages:</strong> ${pageCount}</div><div><strong>ISBN:</strong> ${isbn}</div><div><a href="${goodreadsLink}" target="_blank" class="text-indigo-600 hover:underline font-semibold">Find on Goodreads</a></div></div><div id="reading-log-container" data-book-id="${book.id}" data-shelf="${book.shelf}"></div><div class="mt-6"><h4 class="text-lg font-semibold">Highlights</h4><div id="highlights-container" class="mt-2"></div></div></div></div></div>`; renderReadingLogView(book.id, book.shelf); renderHighlights(book.id, book.shelf);};
    ...
    if (importFileInput) importFileInput.addEventListener('change', (e) => {
        ...
        reader.onload = (event) => {
            try {
                ...
                const allBooks = Object.values(importedLibrary).flat();
                if(allBooks.length === 0) return showToast('Backup file is empty.', 'error');
                
                requestPassword(async () => {
                    showToast(`Importing ${allBooks.length} books... This may take a moment.`, 'success');
                    for (const book of allBooks) {
                        await performAuthenticatedAction({ action: 'add', data: book });
                    }
